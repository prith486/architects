...
export default function HeroScroll({ content = HOMEPAGE_FALLBACK }: HeroScrollProps) {
  ...
  const heroGoldWordStyle: React.CSSProperties = {
  ...
  const heroGoldInlineStyle: React.CSSProperties = {
    fontFamily: 'var(--font-pinyon), cursive',
    color: '#C9A96E',
    fontSize: '1.6em',
    fontStyle: 'normal',
  ...
  const renderHeroTitle = (text: string) => {
    if (text.trim() !== HOMEPAGE_FALLBACK.heroTitle.trim()) {
      return renderLines(text);
    }

    return (
      <>
        Spaces that <span className="gold-a" style={heroGoldWordStyle}>elevate</span>.<br />
        Experiences that <span className="gold-a" style={heroGoldWordStyle}>endure</span>.
      </>
    );
  };

  const renderHeroSubtitle = (text: string) => {
    if (text.trim() !== HOMEPAGE_FALLBACK.heroSubtitle.trim()) {
      return renderLines(text);
    }

    return (
      <>
        We design with <span style={heroGoldInlineStyle}>intention</span>, craft with <span style={heroGoldInlineStyle}>precision</span><br />
        and build places that <span style={heroGoldInlineStyle}>inspire</span> for generations.
      </>
    );
  };

  const renderBrandName = (text: string) => {
    if (text.trim().toUpperCase() !== 'VAASTU') {
      return text;
    }

    return <>V<span className="gold-a">AA</span>STU</>;
  };
    ...
    >
      ...
      <div className="absolute bottom-[4%] right-[2vw] z-20 pointer-events-auto">
        <button className="flex items-center gap-4 px-6 py-[14px] rounded-[30px] bg-black/30 backdrop-blur-md border border-white/10 hover:bg-black/50 transition-colors group shadow-2xl">
          ...
          <span className="font-lato text-[11px] tracking-[0.2em] font-medium text-[#F8F4EC] uppercase mt-[1px]">{content.heroUi.showreelButtonText}</span>
        </button>
      </div>
      ...
      */}
      ...
      <div className="hero-top-bar fade-layer pointer-events-none !justify-center" data-speed="1.5">
        <div className="brand-center">
          <div className="brand-logo">{renderBrandName(content.heroUi.brandName)}</div>
          <div className="brand-tagline">{content.heroUi.brandTagline}</div>
        </div>
      ...
      <div className="hero-main-content fade-layer pointer-events-none relative z-10" data-speed="1">
        ...
        <p className="hero-subtitle" style={{ color: '#EAE3D5', textShadow: '0 2px 8px rgba(0,0,0,0.5)' }}>
          {renderHeroSubtitle(content.heroSubtitle)}
        </p>
        <div className="hero-explore pointer-events-auto">
          <span className="explore-text">{content.heroUi.exploreText}</span>
      ...
      <div className="absolute right-[2vw] top-1/2 -translate-y-1/2 translate-x-1/2 flex flex-col items-center gap-[42px] z-20 pointer-events-auto fade-layer" data-speed="1">
        <a href={content.heroUi.instagramUrl} className="text-[#C4A47C] drop-shadow-[0_0px_6px_rgba(196,164,124,0.7)] hover:text-[#F8F6F2] hover:drop-shadow-[0_0px_12px_rgba(248,246,242,0.9)] hover:scale-110 transition-all duration-[350ms]">
          <InstagramIcon size={28} />
        </a>
        <a href={content.heroUi.linkedinUrl} className="text-[#C4A47C] drop-shadow-[0_0px_6px_rgba(196,164,124,0.7)] hover:text-[#F8F6F2] hover:drop-shadow-[0_0px_12px_rgba(248,246,242,0.9)] hover:scale-110 transition-all duration-[350ms]">
          <LinkedinIcon size={28} />
        </a>
        <a href={content.heroUi.emailUrl} className="text-[#C4A47C] drop-shadow-[0_0px_6px_rgba(196,164,124,0.7)] hover:text-[#F8F6F2] hover:drop-shadow-[0_0px_12px_rgba(248,246,242,0.9)] hover:scale-110 transition-all duration-[350ms]">
          <EmailIcon size={28} />
        </a>
      </div>
        ...
        <div className="absolute top-12 md:top-20 bottom-[120px] left-1/2 -translate-x-1/2 w-full max-w-[1100px] flex flex-col items-center justify-center z-10 px-6 md:px-16">
          ...
          <div className="cta-module flex flex-row items-center justify-center gap-[28px] mt-[20px] flex-shrink-0 pointer-events-auto">
            ...
            <button className="w-[240px] h-[52px] bg-transparent border border-[#C4A47C]/60 text-[#C4A47C] font-lato font-normal text-[12px] uppercase tracking-[0.12em] rounded-none flex items-center justify-center gap-2 transition-all duration-[350ms] ease-[cubic-bezier(0.16,1,0.3,1)] hover:bg-[#C4A47C]/[0.06] group">
              {content.heroUi.secondaryCtaText}
              <span className="transition-transform duration-[350ms] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:translate-x-[6px]">→</span>
            </button>
          </div>
          ...
          <div className="social-module flex flex-row items-center justify-center gap-[48px] mt-[30px] flex-shrink-0 pointer-events-auto">
            <a href={content.heroUi.instagramUrl} className="text-[#C4A47C] hover:text-[#F8F6F2] transition-all duration-[350ms] group drop-shadow-[0_2px_6px_rgba(196,164,124,0.3)] hover:drop-shadow-[0_0px_10px_rgba(248,246,242,0.5)]">
              <InstagramIcon size={32} className="group-hover:scale-110 transition-transform duration-[350ms]" />
            </a>
            <a href={content.heroUi.linkedinUrl} className="text-[#C4A47C] hover:text-[#F8F6F2] transition-all duration-[350ms] group drop-shadow-[0_2px_6px_rgba(196,164,124,0.3)] hover:drop-shadow-[0_0px_10px_rgba(248,246,242,0.5)]">
              <LinkedinIcon size={32} className="group-hover:scale-110 transition-transform duration-[350ms]" />
            </a>
            <a href={content.heroUi.twitterUrl} className="text-[#C4A47C] hover:text-[#F8F6F2] transition-all duration-[350ms] group drop-shadow-[0_2px_6px_rgba(196,164,124,0.3)] hover:drop-shadow-[0_0px_10px_rgba(248,246,242,0.5)]">
