...
import type { ContactFooterData } from "@/sanity/lib/contactFooterMapper";
...
interface ContactSectionProps {
  contactContent?: ContactFooterData;
}

export default function ContactSection({ contactContent }: ContactSectionProps) {
  const content = contactContent?.contactSection;
  const contactSchema = z.object({
    name: z.string().min(2, content?.nameValidationMessage ?? "Name is required"),
    email: z.string().email(content?.emailValidationMessage ?? "Invalid email address"),
  });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        ...
        <div className="flex flex-col z-10">
          <h2 className="text-5xl md:text-7xl font-serif text-[#1a1a1a] leading-tight mb-6">
            {content?.headingLineOne ?? "Let's discuss"} <br />
            <span className="text-[#b7955b] italic">{content?.headingLineTwo ?? "your vision."}</span>
          </h2>

          <div className="flex gap-12 mt-8">
            <div>
              <span className="font-sans text-[10px] tracking-[0.2em] uppercase text-[#b7955b] block mb-2">{content?.studioLabel ?? "Studio"}</span>
              <p className="font-sans text-[#5c5c5c] leading-relaxed">
                {(contactContent?.addressLines ?? ['Pune, Maharashtra', 'India']).map((line, index, lines) => (
                  <span key={`${line}-${index}`}>
                    {line}
            ...
            <div>
              <span className="font-sans text-[10px] tracking-[0.2em] uppercase text-[#b7955b] block mb-2">{content?.inquiriesLabel ?? "Inquiries"}</span>
              <p className="font-sans text-[#5c5c5c] leading-relaxed">
                {contactContent?.email ?? 'hello@vaastu-studio.com'}<br/>
                {contactContent?.phone ?? '+91 98765 43210'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 bg-white/40 backdrop-blur-md border border-black/5 p-10 lg:p-14 shadow-xl">
          ...
          <div className="flex gap-6 mb-10 border-b border-black/10 pb-6">
            ...
            >
              <Mail size={16} /> {content?.requestInfoLabel ?? "Request Info"}
            </button>
            <button
            ...
            >
              <Calendar size={16} /> {content?.bookConsultLabel ?? "Book Consult"}
            </button>
          </div>
            ...
            <AnimatePresence mode="wait">
              ...
              {bookingMode === "form" && !isSubmitted && (
                ...
                >
                  ...
                  >
                    {isSubmitting ? content?.submittingText ?? "Processing..." : content?.submitText ?? "Initiate Contact"}
              ...
              {bookingMode === "form" && isSubmitted && (
                ...
                >
                  ...
                  <h3 className="font-serif text-2xl text-[#1a1a1a] mb-2">{content?.successHeading ?? "Request Received"}</h3>
                  <p className="font-sans text-[#5c5c5c] text-sm">{content?.successDescription ?? "Our lead architect will contact you shortly."}</p>
                </motion.div>
              )}
              ...
              {bookingMode === "calendly" && (
                ...
                >
                  ...
                  <p className="font-sans text-xs tracking-widest uppercase text-[#1a1a1a]/60 mb-2">{content?.calendlyHeading ?? "Select a Time"}</p>
                  <p className="font-sans text-xs text-[#1a1a1a]/40 italic">{content?.calendlyDescription ?? "Calendly embed renders here"}</p>
                </motion.div>
              )}
